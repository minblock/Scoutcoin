...
const char * const BITCOIN_CONF_FILENAME = "scoutcoin.conf";
...
#else
    const char* pszModule = "scoutcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Scoutcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Scoutcoin";
#else
    // Unix
    return pathRet / ".scoutcoin";
#endif
#endif
}
